...
class GetMotorcyclesActionTest extends \PHPUnit_Framework_TestCase
{
    public function testGetReturnsMotorcycle()
    {
        ...
        $id = 1;
        $motorcycleRepository->findOneByBikerAndId($biker, $id)->willReturn(new Motorcycle('model'));
        $getMotorcycleAction = new GetMotorcycleAction($motorcycleRepository->reveal(), $biker->reveal());
